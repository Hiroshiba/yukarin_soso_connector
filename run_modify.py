...
class Mora:
    ...
    def labels(self):
        if self.consonant is not None:
            return [self.consonant.label, self.vowel.label]
        else:
            return [self.vowel.label]
...
class BreathGroup:
    ...
    @property
    def labels(self):
        labels: List[Phoneme] = chain.from_iterable(
...
def run(text: str):
    ...
    x, sr = pyopenjtalk.synthesize(labels, speed=1, half_tone=0)
    x /= 2 ** 16
    soundfile.write("hiho_output_before.wav", x, sr)

    phonemes = [Phoneme.from_label(label=label) for label in labels]

    utt = Utterance.from_phonemes(phonemes)

    # new_accent_phrase = (
    #     utt.breath_groups[1]
    #     .accent_phrases[0]
    #     .merge(utt.breath_groups[1].accent_phrases[1])
    # )
    # new_accent_phrase.accent = 8
    # utt.breath_groups[1].accent_phrases[0:2] = [new_accent_phrase]
