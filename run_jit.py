...
def run(
    ...
    if not with_load:
        ...
        torch.jit.save(forwarder.yukarin_s_forwarder, f"yukarin_s_script_{device}.pt")
        torch.jit.save(forwarder.yukarin_sa_forwarder, f"yukarin_sa_script_{device}.pt")
        torch.jit.save(forwarder.decode_forwarder, f"decode_script_{device}.pt")

    else:
        device = "cuda" if use_gpu else "cpu"
        forwarder = JitForwarder(
            yukarin_s_forwarder=torch.jit.load(f"yukarin_s_script_{device}.pt", device),
            yukarin_sa_forwarder=torch.jit.load(
                f"yukarin_sa_script_{device}.pt", device
            ),
            decode_forwarder=torch.jit.load(f"decode_script_{device}.pt", device),
            device=device,
        )
